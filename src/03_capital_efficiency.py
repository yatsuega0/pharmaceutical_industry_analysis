...
# 必須列の確認
required_cols = [
    "証券コード", "企業名", "売上高", "総資産", "自己資本", "ROA", "ROE"
]
...
# 資本効率テーブルを保存
output_cols = [
    "証券コード", "企業名", "総資産", "自己資本", "ROA", "ROE",
    "自己資本比率", "総資産回転率", "ROE_ROA_gap"
]
...
plt.tight_layout()
plt.savefig(project_root / "output" / "fig_03_roa_vs_roe.png", dpi=150, bbox_inches="tight")
plt.show()
...
summary_stats = report.generate_summary_stats(
    df,
    cols=["ROA", "ROE", "自己資本比率", "総資産回転率", "ROE_ROA_gap"]
)
...
sections = [
    ...
    {
        "heading": "自己資本比率",
        "content": equity_summary
    },
    {
        "heading": "ROE-ROA差分（レバレッジ効果）",
        "content": gap_summary
    },
    {
        "heading": "観察事項",
